...
function View() {
    ...
    this.createTravelAction = function(action) {
        ...
        for(let i = 0; i < keyNames.length; i++) {
            let statName = keyNames[i];
            actionStats += "<div class='bold'>" + cnItem(statName) + "</div> " + (action.stats[statName]*100)+"%<br>";
        }
    ...
    this.createTownInfo = function(action) {
        let totalInfoText =
            "<div class='townInfoContainer showthat' id='infoContainer"+action.varName+"'>" +
                ...
                "<label for='searchToggler"+action.varName+"'> 战利品优先</label>"+
                "<div class='showthis'>" +
                    action.infoText +
                "</div>" +
            "</div><br>";
        ...
        for(let i = 0; i < action.segments; i++) {
            pbars += "<div class='thickProgressBar showthat' "+width+">" +
                        ...
                        "<div class='showthis' id='tooltip"+i+action.varName+"'>" +
                            ...
                            "<div class='bold'>主要属性</div> <div id='mainStat"+i+action.varName+"'></div><br>" +
                            "<div class='bold'>进度</div> <div id='progress"+i+action.varName+"'></div> / <div id='progressNeeded"+i+action.varName+"'></div>" +
                        "</div>" +
                    "</div>";
        }
